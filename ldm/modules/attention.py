...
class CrossAttention(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.):
        ...
        self.attn_map = None  # ⚠️ 核心修改 1：在初始化中为 Hook 注册占位符
    ...
    def forward(self, x, context=None, mask=None):
        ...
        self.attn_map = sim  # ⚠️ 核心修改 2：把最原汁原味、未经过任何压缩扭曲的概率图存下来供 Hook 读取
...
class MultiHeadExternalAttention(nn.Module):
    ...
    def _act_dn(self, x):
        B, C, N = x.shape
        x_scaled = x.view(B, self.num_heads, self.inter_channels // self.num_heads, N) * self.scale
        self.attn_logits = x_scaled        
        attn = F.softmax(x_scaled, dim=-1)         
        self.attn_map = attn                       
...
class DynamicHybridAttention(nn.Module):
    def __init__(self, query_dim, n_heads, d_head, time_embed_dim, 
        ...
        # 提前占位 cache
        self.sa_indices_cache = None
        self.global_alpha_cache = None

    def forward(self, x, t_embedding=None):
        import os
        routing_strategy = os.environ.get("DHA_ROUTING", "dynamic")
        sparsity_ratio = float(os.environ.get("DHA_SPARSITY", "0.5"))
        ...
        if routing_strategy == 'random':
            token_importance = torch.rand((batch_size, seq_len), device=x.device)
        elif routing_strategy == 'static':
            token_importance = torch.linspace(1.0, 0.0, seq_len, device=x.device).unsqueeze(0).repeat(batch_size, 1)
        else:
            raw_scores = self.gating_network(t_embedding)
            local_scores = F.layer_norm(raw_scores, raw_scores.shape[-1:])
            token_importance = torch.einsum("bqd,bd->bq", x, local_scores)
            
        # ======== 2. Hard Routing ========
        k = max(self.min_sa_tokens, int(seq_len * sparsity_ratio))  
        
        if self.training and routing_strategy == 'dynamic':
            noise = torch.randn_like(token_importance) * 0.01
            routing_scores = token_importance + noise
        else:
            routing_scores = token_importance

        _, top_indices = torch.topk(routing_scores, k, dim=1)
        expanded_indices = top_indices.unsqueeze(-1).expand(-1, -1, self.query_dim)

        # ======== 3.(Gather -> SA -> Scatter) ========
        x_important = torch.gather(x, dim=1, index=expanded_indices).contiguous()
        sa_important_out = self.sa_attn(x_important)
    
        sa_out = torch.zeros_like(x).scatter(dim=1, index=expanded_indices, src=sa_important_out)
        ea_out = self.ea_attn(x)  
        

        safe_alpha = torch.sigmoid(self.global_alpha) 
        output = ea_out + safe_alpha * sa_out
